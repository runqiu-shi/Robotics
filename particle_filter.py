...
class ParticleFilter:
    def __init__(self, map, num_particles, translation_variance, rotation_variance, measurement_variance, starting_point):
        self._particles = []
        self._map = map
        ...
        for i in range(0, num_particles):
            self._particles.append(Particle(
                starting_point[0] + np.random.normal(0.0, self._translation_variance),
                starting_point[1] + np.random.normal(0.0, self._translation_variance),
                # choice([-math.pi, -math.pi/2, 0, math.pi/2]),
                np.random.normal(0, self._rotation_variance),
                # 0,
                math.log(p)))
